...
function GameHover( elem, event, divHover, rgHoverData )
{
	...
	else if ( ( !$Hover.is(':visible') || g_HoverState.target != $Elem[0] ) && !oElemState.timer )
	{
		...
		{
			var rgAjaxParams = { u: accountId, l: 'portuguese', ls: languages };
			if ( bPublic )
			{
				// is cc needed?
				rgAjaxParams = {
...
function AddToWishlist( appid, divToHide, divToShowSuccess, divToShowError, navref, divToHide2 )
{
	if ( !g_AccountID )
	{
		ShowAlertDialog( "Inicia sess\u00e3o no Steam", "Deves ter a sess\u00e3o iniciada para poderes adicionar um artigo \u00e0 tua lista de desejos." )
		.done( function() {
		...
		.done( function( data ) {
			...
			if ( data && data.success ) {
				...
				if ( data.saleTaskCompleted ) {
					NewStickerPackModal( 'Adiciona algo à tua lista de desejos' );
				}
			}
			else {
...
function AddToWishlistButton( button, appid, navref )
{
	...
	$J.post( url, {sessionid: g_sessionID, appid: appid} )
		.done( function( data ) {
			...
			{
				$J('span',button).text( "Ups, desculpa!" );
			}
...
function HideRecommendation( type, itemid, divBtn, elemContainer )
{
	...
	new Ajax.Request( 'https://store.steampowered.com/recommended/ignorerecommendation/', {
		...
		onSuccess: function( transport ) {
			...
			{
				if ( $(elemContainer) )
					$(elemContainer).update( '<div class="recommendation_ignored">Muito bem, recomendaremos outra coisa da próxima vez</div>' );
			}
			else
			{
				$(divBtn).show();
...
function EnsureStoreMenuTagsLoaded( strId )
{
	...
	if ( !$Element.data('tags-loaded') )
	{
		...
		$J.get( url, {ll: 'portuguese'} ).done( function( data ) {
			...
			if ( rgYourPopularTags.length > 0 )
			{
				...
				for( var i = 0; i < rgYourPopularTags.length && i < 5; i++ )
				{
					...
					var $Link = $J('<a/>', {'class': 'popup_menu_item', 'href': 'https://store.steampowered.com/tag/pt/' + encodeURIComponent( tag.name ) });
					$Link.text( tag.name );
					$Element.append( $Link );
				}
			}
...
function AddFreeLicense( subid, strDisplayName )
{
	...
	$J.when.apply( $J, posts ).done( function() {
		ShowAlertDialog(
			strDisplayName,
			'%s foi adicionado à tua conta e está agora disponível na tua Biblioteca Steam.'.replace( /%s/, strDisplayName )
		).done( function() {
			window.location.reload();
		});
		if ( typeof GDynamicStore != 'undefined' )
		...
		if ( data && data.purchaseresultdetail == 9 )
			ShowAlertDialog( strDisplayName, 'Este produto já se encontra disponível na tua Biblioteca Steam.' );
		else if ( data && data.purchaseresultdetail == 24 )
			ShowAlertDialog( strDisplayName, 'Precisas de ter o produto base para poderes adicionar este produto à tua conta Steam.' );
		else
			ShowAlertDialog( strDisplayName, 'Ocorreu um problema ao adicionar este produto à tua conta. Tenta novamente mais tarde.' );
	}).always( function () {
...
function ChangeLanguage( strTargetLanguage, bStayOnPage )
{
	...
	.done( function() {
		...
		Modal.Dismiss();
		ShowAlertDialog( 'Alterar idioma', 'Ocorreu um problema ao guardar as tuas alterações. Por favor, tenta de novo mais tarde.' );
	});
}
...
function ShowGamePurchaseDropdown( elemLink, elemPopup )
{
	...
	if ( window.UseTabletScreenMode && window.UseTabletScreenMode() )
	{
		...
		var $Dialog = ShowDialog( 'Seleciona uma opção de compra', $Content.show() );
		$Dialog.AdjustSizing();
	}
	else
	{
...
function FollowCuratorWithCallback( clanID, bFollow, onComplete )
{
	...
	if ( !bHaveUser )
	{
		ShowAlertDialog("Inicia a sess\u00e3o", "Tens de ter a sess\u00e3o iniciada para poderes seguir um curador");
		return;
	}
	...
	$J.post(
		...
		{
			ShowAlertDialog( 'Erro', 'Ocorreu um problema ao tentar seguir o Curador Steam. Motivos possíveis:<ul><li>Não podes seguir um curador que já estás a ignorar.</li><li>Só podes seguir até 100 curadores ao mesmo tempo</li><li>Contas limitadas não podem seguir um curador</li></ul>' );
		}
	);
	return false;
}
...
function IgnoreCuratorWithCallback( clanID, bIgnore, onComplete )
{
	...
	if ( !bHaveUser )
	{
		ShowAlertDialog("Inicia a sess\u00e3o", "\u00c9 necess\u00e1rio que tenhas a sess\u00e3o iniciada para poderes ignorar um Curador Steam");
		return;
	}
	...
	$J.post(
		...
		{
			ShowAlertDialog( 'Erro', 'Ocorreu um problema ao tentar ignorar o Curador Steam.<br>Nota: Não podes ignorar um curador que estejas a seguir.' );
		}
	);
	return false;
}
