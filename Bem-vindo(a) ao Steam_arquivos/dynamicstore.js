...
GDynamicStore = {
		...
		$DynamicElements.each( function() {
			...
			if ( !$El.hasClass('ds_no_flags') )
			{
				...
				if ( bOwned )
				{
					...
					$El.append( '<div class="ds_flag ds_owned_flag">JÁ TENS&nbsp;&nbsp;</div>');
				}
				else if ( bWanted )
				{
				...
				if ( $El.hasClass( 'ds_excluded_blur' ) )
				{
                	...
                	$El.append( '<div class="ds_flag ds_excluded_by_preferences_flag">EXCLUÍDO POR PREFERÊNCIAS&nbsp;&nbsp;</div>' );
				}
				...
				{
					$El.addClass( 'ds_flagged ds_excluded_by_preferences' );
					$El.append( '<div class="ds_flag ds_excluded_by_preferences_flag">Tem marcadores excluídos:&nbsp;' + rgExcludedTagNames.join(", " ) + '</div>' );
				}
				else if ( rgExcludedCreatorIDs.length != 0 )
				{
					$El.addClass( 'ds_flagged ds_excluded_by_preferences' );
					$El.append( '<div class="ds_flag ds_excluded_by_preferences_flag">EXCLUÍDO: IGNORAS DEVELOPER / EDITORA / SÉRIE&nbsp;&nbsp;</div>' );
				}
	...
	UpdateDynamicBundleElements: function( Bundle, $El )
	{
		...
		{
			var strTooltip = 'Não podes comprar este conjunto para a tua conta porque já tens todos os artigos incluídos.';

			if ( Bundle.m_bMustPurchaseAsSet && !Bundle.m_bRestrictGifting )
			{
				strTooltip = 'Esta oferta só está disponível ao comprar todos os %s artigos ao mesmo tempo. Também podes comprar o conjunto como presente para um amigo.'.replace(/%s/, Bundle.m_cTotalItemsInBundle);
		...
		else if ( !Bundle.m_bMustPurchaseAsSet )
		{
			...
			if ( !Bundle.m_bContainsDiscountedPackage )
			{
				...
				$DiscountBlocks.find('.discount_final_price' ).addClass('your_price' ).empty().append($J('<div/>', {'class': 'your_price_label'} ).text('Preço para ti:'), $J('<div/>' ).text( strFormattedFinalPrice ) );
			}
			else
			{
		...
		if ( $El.is( '.package_totals_area' ) && !Bundle.m_bMustPurchaseAsSet )
		{
			...
			else
			{
				...
				{
					$El.find('.bundle_final_package_price_desc' ).text( 'Preço individual dos artigos (%s) que ainda não tens:'.replace( '%s', Bundle.m_cUserItemsInBundle ) );
				}
	...
	BuildBundleDescription: function( Bundle, $Description )
	{
		if ( Bundle.m_cUserItemsInBundle == 0 )
		{
			...
			$Description.html( '<span class="collectionComplete">Coleção completa!</span> %1$s de %2$s artigos desta coleção já estão na tua biblioteca.'.replace( '%1$s', Bundle.m_cTotalItemsInBundle ).replace( '%2$s', Bundle.m_cTotalItemsInBundle ) );
		}
		else if ( Bundle.m_cUserItemsInBundle < Bundle.m_cTotalItemsInBundle )
		{
			// own some but not all.
			$Description.html( '<div>%1$s dos %2$s artigos deste conjunto já estão na tua biblioteca.</div>'.replace( '%1$s', Bundle.m_cTotalItemsInBundle - Bundle.m_cUserItemsInBundle ).replace( '%2$s', Bundle.m_cTotalItemsInBundle ) );
			$Description.append( '<div>Compra este conjunto e poupa %1$s%% nos artigos (%2$s) que ainda não tens!</div>'.replace( '%1$s', Math.round( Bundle.m_nDiscountPct ) ).replace( '%2$s', Bundle.m_cUserItemsInBundle ).replace( '%%', '%' ) );

			// add "complete the set" flag
			$Description.parents('.dynamic_bundle_description' ).append( $J('<div/>', {'class': 'ds_flag ds_completetheset'} ).text('COMPLETA A TUA COLEÇÃO!') );
		}
		else
		{
			if ( Math.round( Bundle.m_nDiscountPct ) == 0 )
			{
				$Description.html( '<div>Compra este conjunto para obteres todos os %1$s artigos!</div>'.replace( '%1$s', Bundle.m_cTotalItemsInBundle ) );
			}
			else {
				$Description.html( '<div>Compra este conjunto e poupa %1$s%% em %2$s artigos!</div>'.replace( '%1$s', Math.round( Bundle.m_nDiscountPct ) ).replace( '%2$s', Bundle.m_cTotalItemsInBundle ).replace( '%%', '%' ) );
			}
		}
	...
	PopulateRecommendedTagList: function()
	{
		...
		for( var i = 0; i < GDynamicStore.s_rgRecommendedTags.length && i < 4; i++ )
		{
			...
			var url = 'https://store.steampowered.com/tags/pt/' + encodeURIComponent( tag.name );
	...
	DisplayBundleSimulator: function( unBundleID )
	{
		...
		$Form.append( $J('<h2/>').text('Marca quais produtos "adquirir"') );
		$Form.append( $J('<div/>', {'class': 'bundle_simulator_secondary' } ).append(
			$J('<a/>', {'href': 'javascript:void(0);'} ).click( function() { $Form.find('input').prop('checked', true ); } ).text( "Selecionar tudo" ),
			' - ',
			$J('<a/>', {'href': 'javascript:void(0);'} ).click( function() { $Form.find('input').prop('checked', false ); } ).text( "Não selecionar nenhum" )
		) );
		...
		Modal = ShowConfirmDialog( '', $Form, 'Atualizar aspeto' ).done( function() {
...
var GDynamicStoreHelpers = {
	...
	BuildCapsuleHTML: function( strFeatureContext, unAppID, unPackageID, unBundleID, rgOptions )
	{
		...
		if( rgItemData.has_live_broadcast )
			$CapCtn.append( $J( '<div>').addClass( 'broadcast_live_stream_icon' ).append( 'Em direto' ) );
...
function ShowHowDoDiscoveryQueuesWorkDialog()
{
	$J.get(
		...
		function( data )
		{
			ShowAlertDialog( 'Como é que isto funciona?', data );
		}
	);
}
...
function UpdateStoreBannerForAdditionalCartDiscount( nCartDiscount )
{
	...
	<div class="placeHolder_lunarSale2019_giftActiveBar">	\
		...
		<div class="lunar_sale_title"><img src="https://store.fastly.steamstatic.com/public/images/promo/lunar2019/lny2019_title_pt.png"/></div> \
	...
	var strHeader = 'Modo Super Poupança ativo';
	var strDiscount = 'Um desconto de %amount% será aplicado ao teu carrinho!'.replace( '%amount%', strAmount );
...
function UpdateStoreBannerForPromotionalDiscount( nDiscount, nMinCartAmount, nAvailableUseCount )
{
	...
	var strTitle = 'Desconto das férias grandes';
	var strHeader = 'Poupa %amount% numa compra de %min_amount% ou mais'.replace( '%amount%', strAmount ).replace( '%min_amount%', strMinAmount );
	var strDiscount = 'Desconto aplicado durante o checkout';
...
function BuildCreatorCapsuleToAppend( oItem )
{
	...
	$FollowLink.bind('updateFollowState', function () {
		...
		{
			$FollowLink.html( '<span><img src="https://store.fastly.steamstatic.com/public/images/v6/ico/ico_selected_green.png">A seguir</span>' );
