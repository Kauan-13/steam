...
GHomepage = {
	...
	BuildMainCapsuleItem: function( rgItem, strFeatureContext, nDepth )
	{
		...
		{
			$ImgCtn.append( $J('<div/>', {'class': 'broadcast_live_stream_icon' } ).append( 'Em direto') );
		}
		...
		else if( rgRecommendationReasons.recent_release_by_creator && rgRecommendationReasons.recent_release_by_creator.creator_info )
		{
			...
			var $ReasonLocal = creator.link.indexOf( 'developer/' ) >= 0 ? "<strong>Developer:<\/strong><br><span>%1$s<\/span>" : "<strong>Editora:<\/strong><br><span>%1$s<\/span>";
		...
		else if( rgRecommendationReasons.recommended && GDynamicStore.s_rgRecommendedTags )
		{
			...
			if( rgMatchedTags.length == 0 )
				var $ReasonMain = $J('<div/>').addClass('main').addClass('bytags').html( "<strong>Recomendado<\/strong> porque jogas jogos semelhantes" );
			else
				var $ReasonMain = $J('<div/>').addClass('main').addClass('bytags').html( "<strong>Recomendado<\/strong> porque jogaste jogos com o(s) marcador(es):" );
		...
		if( bShowAdditionalReasons )
		{
			...
			for( var key in rgRecommendationReasons )
			{
				...
				if( key == 'top_seller' )
					...
					strReason = "Novo lan\u00e7amento";
				else if( key == 'recommended')
					strReason = "Semelhante ao que jogas";
				else
					continue;
				...
				$el.on('hover focus', function(){
					...
					else
					{
						...
						if ( rgData['review_summary'] )
						{
							...
							$elReviewData.append( $J('<div>', {'class': 'title'}).text( rgData.appids ? 'Análises de utilizadores dos produtos neste conjunto:' : 'Análises de utilizadores no geral:') );
						...
						if( rgData.tags )
						{
							...
							{
								var url = GStoreItemData.AddNavEventParamsToURL( 'https://store.steampowered.com/tags/pt/TAGNAME/'.replace( /TAGNAME/, encodeURIComponent( rgData.tags[i] ) ), 'tab_preview' );
						...
						for( var i=0; i < 4; i++ )
						{
							...
							{
								var $elScreenshot = $J('<div>', {'class': 'screenshot empty'}).append("<div>").text("Esta imagem pode conter temas adultos.");
								$elInfoDiv.append($elScreenshot);
							}
						}
	...
	RenderRecommendedByDeepDiveCarousel: function()
	{
		...
		focusedAppTitle = 'Porque jogaste <strong>%s</strong>'.replace( "%s", focusedAppTitle);
	...
	RenderCommunityRecommendations: function()
	{
		...
		{
			elemPrefLastUpdated.text( 'A usar as preferências guardadas em %1$s'.replace( "%1$s", GHomepage.strCommunityRecommendationsPrefLastSaved ) );
		}
	...
	BuildHomePageGenericCap: function( strFeatureContext, unAppID, unPackageID, unBundleID, rgOptions, nDepth )
	{
		...
		{
			$CapCtn.append(
					$J('<div/>', {'class': 'broadcast_live_stream_icon' } ).append( 'Em direto')
			);
		}
	...
	BuildCommunityRecommendationCap: function( strFeatureContext, oItem )
	{
		...
		if ( numReviews > 1 )
		{
			...
			let strNumReviews = '%1$s de %2$s análises'.replace( "%1$s", 1 ).replace( "%2$s", numReviews );
			...
			arrowLeft.click( function() {
				...
				var strNumReviews = '%1$s de %2$s análises'.replace( "%1$s", curIdx + 1 ).replace( "%2$s", numReviews );
		...
		for( var i = 0; i < rgReviews.length; ++i)
		{
			...
			if ( reviewer )
			{
				...
				{
					var strHours = 'Com %s horas de jogo no momento da análise'.replace( "%s", v_numberformat( review.playtime_at_review / 60, 1 ) );
	...
	RenderRecommendedTags: function()
	{
		...
		for ( var i = 0; i < GDynamicStore.s_rgRecommendedTags.length; i++ )
		{
			...
			{
				var url = GStoreItemData.AddNavEventParamsToURL( 'https://store.steampowered.com/tags/pt/TAGNAME/'.replace( /TAGNAME/, encodeURIComponent( rgTag.name ) ), 'gutter' );
...
CHomeSettings.prototype.DisplayPopup = function( $Btn )
{
	...
	this.m_$Popup.append( $J('<div/>', {'class': 'home_viewsettings_instructions' } ).text( 'Seleciona os tipos de produtos que desejas ver nesta secção' ) );

	if ( this.m_ApplicableSettings.popular_new_releases )
		this.m_$Popup.append( this.RenderCheckbox( 'popular_new_releases', 'Novidades populares' ) );
	...
	if ( this.m_ApplicableSettings.early_access )
		this.m_$Popup.append( this.RenderCheckbox( 'early_access', 'Produtos de Acesso Antecipado' ) );
	if ( this.m_ApplicableSettings.games_already_in_library )
		this.m_$Popup.append( this.RenderCheckbox( 'games_already_in_library', 'Jogos já na tua conta' ) );
	if ( this.m_ApplicableSettings.games_not_in_library )
		this.m_$Popup.append( this.RenderCheckbox( 'games_not_in_library', 'Jogos que não estão na tua conta' ) );
	...
	if ( this.m_ApplicableSettings.prepurchase )
		this.m_$Popup.append( this.RenderCheckbox( 'prepurchase', 'Pré-reservas' ) );
	...
	if ( this.m_ApplicableSettings.dlc_for_you )
		this.m_$Popup.append( this.RenderCheckbox( 'dlc_for_you', 'DLC para os teus jogos e software' ) );
	if ( this.m_ApplicableSettings.recently_viewed )
		this.m_$Popup.append( this.RenderCheckbox( 'recently_viewed', 'Produtos que viste recentemente' ) );
	...
	if ( this.m_ApplicableSettings.dlc )
		this.m_$Popup.append( this.RenderCheckbox( 'dlc', 'Conteúdo transferível' ) );
	...
	if ( this.m_ApplicableSettings.recommended_by_curators )
		this.m_$Popup.append( this.RenderCheckbox( 'recommended_by_curators', 'Recomendações dos curadores que segues' ) );
	...
	if ( this.m_ApplicableSettings.only_current_platform )
	{
		...
		if ( GDynamicStore.s_bUserOnLinux )
			...
			this.m_$Popup.append( this.RenderCheckbox( 'only_current_platform', 'Disponível para Mac' ) );
	}
...
CHomeSettings.prototype.OnCheckboxToggle = function( strSettingName, $Checkbox )
{
	...
	$J.post( 'https://store.steampowered.com/dynamicstore/updatehomeviewsettings', {
		...
		ShowAlertDialog( 'Personalizar', 'Ocorreu um problema ao guardar as tuas preferências. Tenta de novo mais tarde.' );
	} );
};
			 ...
			 GSteamCurators.rgAppsRecommendedByCurators.apps.length != 0 )
		{
			...
			if ( rgRecommendedApps.length >= 5 )
			{
				...
				for( var j=0; j<rgRecommendedApps.length; j++ in rgRecommendedApps )
				{
					...
					var $elButtonWishlist = $J('<span />').text("Adicionar \u00e0 tua lista de desejos");
